...
if __name__ == '__main__':
    ...
    for i in range(len(train)):
        predicted = classifier.predict(scaler.transform([train_x[i]]))[0]
        if predicted == train_y[i]:
            acc1 += 1

    acc2 = 0
    for i in range(len(test)):
        predicted = classifier.predict(scaler.transform([test_x[i]]))[0]
    ...
    if acc1 > acc2 * 1.15:
        ...
        scaler.fit(train_x)
        classifier.fit(scaler.transform(train_x), train_y)
        print(classifier.predict(scaler.transform([input_string]))[0])
    else:
