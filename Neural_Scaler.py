...
if __name__ == '__main__':
    ...
    neural_network.fit(scaler.transform(train_x), train_y)

    acc1, acc2 = 0, 0

    for i in range(len(train_x)):
        predicted = neural_network.predict(scaler.transform([train_x[i]]))[0]
        if train_y[i] == predicted:
            acc1 += 1

    for i in range(len(test_x)):
        predicted = neural_network.predict(scaler.transform([test_x[i]]))[0]
        if test_y[i] == predicted:
            acc2 += 1

    input_string = [float(el) for el in input().split(' ')]

    if acc1/len(train_x) >= acc2/len(test_x) * 1.15:
        print('Se sluchuva overfitting')
        input_string.pop(col)

        scaler2 = MinMaxScaler(feature_range=(-1, 1))
        neural_network2 = MLPClassifier(hidden_layer_sizes=n,
                                        learning_rate_init=l_rate,
                                        activation='relu',
                                        max_iter=20,
                                        random_state=0)
        ...
        scaler2.fit(train_x)
        neural_network2.fit(scaler2.transform(train_x), train_y)

        print(neural_network2.predict(scaler2.transform([input_string]))[0])
    else:
        print('Ne se sluchuva overfitting')
        print(neural_network.predict(scaler.transform([input_string]))[0])
