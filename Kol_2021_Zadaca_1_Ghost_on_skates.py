...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        x, y = state
        if x < n and y < n and state not in walls:
            return state
        return False
    ...
    def successor(self, state):
        ...
        for i in range(1, 4):
            # Gore
            new_ghost = GhostOnSkates.check_valid((x, y + i), self.walls, self.n)
            if new_ghost:
                successors[f'Gore {i}'] = new_ghost
            # Desno
            new_ghost = GhostOnSkates.check_valid((x + i, y), self.walls, self.n)
            if new_ghost:
                successors[f'Desno {i}'] = new_ghost

        return successors

    def h(self, node):
        return (abs(node.state[0] - goal_pos[0])
                + abs(node.state[1] - goal_pos[1])) / 3
