...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        i = 1
        for x, y in state:
            new_x, new_y = gore(x, y)
            if new_y != y or new_x != x:
                new_state = list(state)
                new_state[i - 1] = list(new_state[i - 1])
                new_state[i - 1][1] = new_y
                new_state[i - 1] = tuple(new_state[i - 1])
                new_state = tuple(new_state)
                successors[f'Pomesti kvadratche {i} gore'] = new_state

            new_x, new_y = dolu(x, y)
            if new_y != y or new_x != x:
                new_state = list(state)
                new_state[i - 1] = list(new_state[i - 1])
                new_state[i - 1][1] = new_y
                new_state[i - 1] = tuple(new_state[i - 1])
                new_state = tuple(new_state)
                successors[f'Pomesti kvadratche {i} dolu'] = new_state

            new_x, new_y = levo(x, y)
            if new_y != y or new_x != x:
                new_state = list(state)
                new_state[i - 1] = list(new_state[i - 1])
                new_state[i - 1][0] = new_x
                new_state[i - 1] = tuple(new_state[i - 1])
                new_state = tuple(new_state)
                successors[f'Pomesti kvadratche {i} levo'] = new_state

            new_x, new_y = desno(x, y)
            if new_y != y or new_x != x:
                new_state = list(state)
                new_state[i - 1] = list(new_state[i - 1])
                new_state[i - 1][0] = new_x
                new_state[i - 1] = tuple(new_state[i - 1])
                new_state = tuple(new_state)
                successors[f'Pomesti kvadratche {i} desno'] = new_state
            i = i + 1
