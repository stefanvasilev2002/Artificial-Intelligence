...
Првите X проценти од податочното множество се користат за тестирање, а останатите за тренирање. Делот од множеството 
што треба да се користи за тестирање се чита од стандардниот влез.

Истренирајте дрво на одлука (criterion='gini') и најдете го атрибутот со најмала важност. Овој атрибут отстранете го 
...
Потребно е да ги пресметате и испечатите точноста на класификаторот трениран со податоците скалирани со StandardScaler 
и на класификаторот трениран со податоците скалирани со MinMaxScaler.
...
if __name__ == '__main__':
    ...
    test = dataset[:int(len(dataset) * x)]
    train = dataset[int(len(dataset) * x):]
    ...
    for row in train_x:
        row.pop(least_important)
    for row in test_x:
        row.pop(least_important)

    neural_network_1 = MLPClassifier(hidden_layer_sizes=15,
                                     activation='relu',
                                     learning_rate_init=0.001,
                                     max_iter=200,
                                     random_state=0)
    neural_network_2 = MLPClassifier(hidden_layer_sizes=15,
                                     activation='relu',
                                     learning_rate_init=0.001,
                                     max_iter=200,
                                     random_state=0)
    ...
    neural_network_1.fit(minMax.transform(train_x), train_y)
    neural_network_2.fit(standard.transform(train_x), train_y)

    acc1, acc2 = 0, 0

    for i in range(0, len(test_x)):
        predicted = neural_network_1.predict(minMax.transform([test_x[i]]))[0]
        if predicted == test_y[i]:
            acc1 += 1
    for i in range(0, len(test_x)):
        predicted = neural_network_2.predict(standard.transform([test_x[i]]))[0]
        if predicted == test_y[i]:
            acc2 += 1

    print(f'Tocnost so StandardScaler: {acc2 / len(test_x)}')
    print(f'Tocnost so MinMaxScaler: {acc1 / len(test_x)}')
