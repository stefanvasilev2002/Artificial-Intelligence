...
if __name__ == '__main__':
    ...
    train1 = class2[:int(len(class2) * x1)] \
             + class1[:int(len(class1) * x1)] \
             + class0[:int(len(class0) * x1)]

    train2 = class2[int(len(class2) * x1):int(len(class2) * x2)] \
             + class1[int(len(class1) * x1):int(len(class1) * x2)] \
             + class0[int(len(class0) * x1):int(len(class0) * x2)]

    train3 = class2[:int(len(class2) * x2)] \
             + class1[:int(len(class1) * x2)] \
             + class0[:int(len(class0) * x2)]
    test = class2[int(len(class2) * x2):] \
           + class1[int(len(class1) * x2):] \
           + class0[int(len(class0) * x2):]

    train1_x = [row[:-1] for row in train1]
    train1_y = [row[-1] for row in train1]

    train2_x = [row[:-1] for row in train2]
    train2_y = [row[-1] for row in train2]

    train3_x = [row[:-1] for row in train3]
    train3_y = [row[-1] for row in train3]

    test_x = [row[:-1] for row in test]
    test_y = [row[-1] for row in test]

    naive_bayes.fit(train1_x, train1_y)
    tree.fit(train2_x, train2_y)
    forest.fit(train3_x, train3_y)

    acc = 0

    for i in range(len(test_x)):
        pred1 = naive_bayes.predict([test_x[i]])[0]
        pred2 = tree.predict([test_x[i]])[0]
        pred3 = forest.predict([test_x[i]])[0]

        if (pred1 == test_y[i] and pred2 == test_y[i]) or \
                (pred1 == test_y[i] and pred3 == test_y[i]) or \
                (pred2 == test_y[i] and pred3 == test_y[i]):
            acc += 1

    print(f'Tochnost: {acc/len(test_x)}')
