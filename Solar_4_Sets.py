...
"""
Дадено е податочно множество за класификација на соларни сигнали, кое се состои од 
15 карактеристики и две класи.

Потребно е да направите евалуација на модел на класификација преку методот на крос 
валидација. За таа цел, поделете го податочното множество на 4 подмножества:
...
Потоа, извршете 4 чекори на тренирање на класификаторот така што секое подмножество еднаш 
ќе се користи за тестирање, а останатите 3 за тренирање. На пример: во првиот чекор за 
тренирање ги користите П2, П3 и П4, додека за тестирање го користите П1. Во секој чекор 
пресметајте ја точноста на моделот. Испечатете ја просечната точност од сите чекори.

Со поделбата на подмножества со која се добива највисока точност направете евалуација со 
отстранување на колона чиј индекс се чита од стандарден влез. На пример: ако највисока точност 
се добива кога за тренирање се користат П1, П2 и П3, а за тестирање П4 тогаш од ваквата верзија 
на подмножества за тренирање и тестирање ја отстранувате колоната. Повторно извршете тренирање 
и евалуација на класификаторот. Испечатете ја неговата точност.

Од стандарден влез прво се чита името на класификаторот (model) кој треба да се евалуира 
(NB за наивен баесов класификатор; MLP за невронска мрежа со 50 неврони, ReLU активациска функција 
и 0.001 рата на учење), а потоа и колона која треба да се отстрани (col).
"""
...
if __name__ == '__main__':
    ...
    if model == 'NB':
        ...
        classifier1 = MLPClassifier(hidden_layer_sizes=50, activation='relu', learning_rate_init=0.001, random_state=0)
        classifier2 = MLPClassifier(hidden_layer_sizes=50, activation='relu', learning_rate_init=0.001, random_state=0)
        classifier3 = MLPClassifier(hidden_layer_sizes=50, activation='relu', learning_rate_init=0.001, random_state=0)
        classifier4 = MLPClassifier(hidden_layer_sizes=50, activation='relu', learning_rate_init=0.001, random_state=0)

    data_1 = [row for row in data if row[-1] == 1]
    data_0 = [row for row in data if row[-1] == 0]

    set_1 = data_1[:int(0.25 * len(data_1))] + data_0[:int(0.25 * len(data_0))]
    set_2 = data_1[int(0.25 * len(data_1)):int(0.5 * len(data_1))] + data_0[
                                                                     int(0.25 * len(data_0)):int(0.5 * len(data_0))]
    set_3 = data_1[int(0.5 * len(data_1)):int(0.75 * len(data_1))] + data_0[
                                                                     int(0.5 * len(data_0)):int(0.75 * len(data_0))]
    set_4 = data_1[int(0.75 * len(data_1)):] + data_0[int(0.75 * len(data_0)):]

    set_1_x = [row[:-1] for row in set_1]
    set_1_y = [row[-1] for row in set_1]

    set_2_x = [row[:-1] for row in set_2]
    set_2_y = [row[-1] for row in set_2]

    set_3_x = [row[:-1] for row in set_3]
    set_3_y = [row[-1] for row in set_3]

    set_4_x = [row[:-1] for row in set_4]
    set_4_y = [row[-1] for row in set_4]

    classifier1.fit(set_2_x + set_3_x + set_4_x, set_2_y + set_3_y + set_4_y)
    classifier2.fit(set_1_x + set_3_x + set_4_x, set_1_y + set_3_y + set_4_y)
    classifier3.fit(set_1_x + set_2_x + set_4_x, set_1_y + set_2_y + set_4_y)
    classifier4.fit(set_1_x + set_2_x + set_3_x, set_1_y + set_2_y + set_3_y)

    acc1, acc2, acc3, acc4 = 0, 0, 0, 0

    for i in range(0, len(set_1_x)):
        if set_1_y[i] == classifier1.predict([set_1_x[i]])[0]:
            acc1 += 1
    for i in range(0, len(set_2_x)):
        if set_2_y[i] == classifier2.predict([set_2_x[i]])[0]:
            acc2 += 1
    for i in range(0, len(set_3_x)):
        if set_3_y[i] == classifier3.predict([set_3_x[i]])[0]:
            acc3 += 1
    for i in range(0, len(set_4_x)):
        if set_4_y[i] == classifier4.predict([set_4_x[i]])[0]:
            acc4 += 1
    print(f'Prosechna tochnost: {(acc1 / len(set_1) + acc2 / len(set_2) + acc3 / len(set_3) + acc4 / len(set_4)) / 4}')
    ...
    if acc1 > acc2 and acc1 > acc3 and acc1 > acc4:
        train_x = set_2_x + set_3_x + set_4_x
        train_y = set_2_y + set_3_y + set_4_y
        test_x = set_1_x
        test_y = set_1_y
        # print(f'Tochnost so otstraneta kolona: {acc1}')
    elif acc2 > acc1 and acc2 > acc3 and acc2 > acc4:
        train_x = set_1_x + set_3_x + set_4_x
        train_y = set_1_y + set_3_y + set_4_y
        test_x = set_2_x
        test_y = set_2_y
        # print(f'Tochnost so otstraneta kolona: {acc2}')
    elif acc3 > acc1 and acc3 > acc2 and acc3 > acc4:
        train_x = set_1_x + set_2_x + set_4_x
        train_y = set_1_y + set_2_y + set_4_y
        test_x = set_3_x
        test_y = set_3_y
        # print(f'Tochnost so otstraneta kolona: {acc3}')
    elif acc4 > acc1 and acc4 > acc2 and acc4 > acc3:
        train_x = set_1_x + set_2_x + set_3_x
        train_y = set_1_y + set_2_y + set_3_y
        test_x = set_4_x
        test_y = set_4_y
        # print(f'Tochnost so otstraneta kolona: {acc4}')
    ...
    for i in range(0, len(test_x)):
        if test_y[i] == classifier.predict([test_x[i]])[0]:
            acc += 1
    print(f'Tochnost so otstraneta kolona: {acc / len(test_x)}')
