...
class Disc(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(0, len(curr)):
            new_state = D1(list(state), i)
            if new_state != curr:
                successors[f'D1: Disk {curr[i]}'] = new_state

            new_state = D2(list(state), i)
            if new_state != curr:
                successors[f'D2: Disk {curr[i]}'] = new_state

            new_state = L1(list(state), i)
            if new_state != curr:
                successors[f'L1: Disk {curr[i]}'] = new_state

            new_state = L2(list(state), i)
            if new_state != curr:
                successors[f'L2: Disk {curr[i]}'] = new_state
...
def D2(tmp, i):
    d = tmp
    if i + 2 < len(d):
        if d[i + 1] != ' ' and d[i + 2] == ' ' and d[i].isnumeric:
            d[i + 2] = d[i]
            d[i] = ' '
    return tuple(d)
...
if __name__ == "__main__":
    ...
    d = Disc((tuple(a)), tuple(goal))
    print(breadth_first_graph_search(d).solution())
    # print(astar_search(d).solution())
