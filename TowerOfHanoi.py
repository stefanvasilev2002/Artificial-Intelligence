    ...
    def successor(self, state):
        ...
        for i in range(0, len(curr)):
            ...
            for j in range(0, len(curr)):
                ...
                if len(curr[i]) == 0:
                    continue
                if len(curr[j]) == 0 or curr[i][-1] <= curr[j][-1]:
                    curr = list(curr)
                    curr[i] = list(curr[i])
                    curr[j] = list(curr[j])
                    curr[j].append(curr[i].pop())
                    curr[i] = tuple(curr[i])
                    curr[j] = tuple(curr[j])
                    successors[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = tuple(curr)
...
if __name__ == "__main__":
    ...
    for i in init:
        if i == ';':
            if len(num) != 0:
                start_hanoi.append(tuple(num))
                num = []
            start_hanoi.append(tuple())
        elif i != ',':
            num.append(int(i))
    if len(num) != 0:
        start_hanoi.append(tuple(num))
    ...
    for i in goal:
        if i == ';':
            if len(num) != 0:
                goal_hanoi.append(tuple(num))
                num = []
            goal_hanoi.append(tuple())
        elif i != ',':
            num.append(int(i))
    if len(num) != 0:
        goal_hanoi.append(tuple(num))

    hanoi = Hanoi(tuple(start_hanoi), tuple(goal_hanoi))
    print(f'Number of action {len(breadth_first_graph_search(hanoi).solution())}')
