...
if __name__ == '__main__':
    ...
    for i in range(0, len(test_x)):
        if test_y[i] == gaussian.predict([test_x[i]])[0]:
            acc += 1

    print(acc/len(test_x))

    input_string = input().split(' ')
    input_string = [float(el) for el in input_string]
    print(int(gaussian.predict([input_string])[0]))
    print(gaussian.predict_proba([input_string]))
